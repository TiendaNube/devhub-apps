...
export default function errorMiddleware(
  ...
  if (
    ...
    !process.env.AUTHENTICATION_API
  ) {
    return res
  ...
  if (req.path === "/products" || req.path === "/products/total") {
    if (!credentials) {
      return {
        ...
        data: "Not found credentials properties at db.json",
      };
    }
    ...
    const accessTokenHeader = authenticationHeader?.startsWith("bearer")
      ? authenticationHeader.slice(7)
      : undefined;

    if (accessTokenHeader !== credentials.access_token) {
      return res
        .status(StatusCode.UNAUTHORIZED)
        .json("The access_token of the Authentication is invalid");
    }
  }
