...
class AuthenticationAppService {
  public async auth(code: string): Promise<TiendanubeAuthInterface> {
      if (!code) {
        const credentials = userRepository.findCredentials();
        if (credentials) {
          return credentials;
        }
      ...
      if (
        ...
        throw new BadRequestException(authenticateResponse.error as string, authenticateResponse.error_description)
      }

      // Insert response of Authentication API at db.json file
      userRepository.saveCredentials(authenticateResponse);
